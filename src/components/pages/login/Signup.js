...
const Signup = () => {
  ...
  const onClickVerify = (verifyCode) => {
    // 인증코드 값을 보낸 후 백엔드에서 일치하는지 처리
   //백엔드에서 일치한다고 답이 오면 계속하기 버튼 활성화
   //인증 된 경우
   setEmailAdd(email.current.value)
  };
  const google = async () => {
    try {
      ...
      >
        ...
        <div className='inputForm'>
            <div style={{display:"flex",marginLeft:"-70px"}}>
              ...
              <button type="button" className="sendBtn" disabled={sendBtnDisabled} style={{margin:"3px"}} onClick={()=>{OnClickSend(email.current.value)}}>{timeFormat(time)}</button>
            </div>
            <br/>
            <div style={{display:"flex",marginLeft:"-70px"}}>
              <p style={{width:"150px"}}>인증 코드</p>
              <input type='text' ref={verifyCode} required></input>
              <button type="button" disabled={verifyBtnDisabled} style={{margin:"3px"}} onClick={()=>{onClickVerify(verifyCode.current.value)}}>인증 하기</button>
            </div>
          </div>

            <Link to={`/Signupform/${emailAdd}`} onClick={onClickLogin}>
